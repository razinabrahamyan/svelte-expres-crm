<script lang="ts">
    ...
    import {saveData} from "../../../utils/utils_svelte";
    ...
    function saveImage() {
        ...
        blurs.forEach(element => {
            blurAreas.push({
                ...
                top: ((element.top.value * height) / 200).toFixed(0),
                width: ((element.width.value * width) / 400).toFixed(0),
                height: ((element.height.value * height) / 200).toFixed(0)
            })
        })
        ...
        formData.append('crop_top', ((crop_top.value * height) / 200).toFixed(0))
        formData.append('crop_bottom', ((crop_bottom.value * height) / 200).toFixed(0))
...
<input use:setFile
       ...
       type="file" />
<Cropper image={widgetValue?.[0]} bind:rotate={rotate} bind:blurs={blurs} bind:crop_left={crop_left} bind:crop_right={crop_right} bind:crop_top={crop_top} bind:crop_bottom={crop_bottom}/>
